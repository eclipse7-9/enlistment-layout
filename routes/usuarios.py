...
def update_usuario(usuario_id: int, request: UsuarioUpdate, db: Session = Depends(get_db)):
    ...
    # Recorre todos los campos del modelo y los actualiza si no son None
    for campo, valor in request.dict(exclude_unset=True).items():
        setattr(usuario, campo, valor)

    db.commit()
    db.refresh(usuario)
    return {"msg": "Usuario actualizado correctamente", "usuario": usuario.correo_usuario}
...
def login_user(request: LoginRequest, db: Session = Depends(get_db)):
    ...
    if user.estado_usuario.lower() == "inactivo":
        return {
            "estado_usuario": "inactivo",
            "msg": "Tu cuenta está inactiva. Contacta al administrador."
        }
    ...
    return {
        ...
        "id_rol": user.id_rol,
        "estado_usuario": user.estado_usuario,  # <-- Importante para React
    }
