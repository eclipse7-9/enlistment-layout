...
const PLACEHOLDER_SVG = "data:image/svg+xml;utf8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='600' height='400'%3E%3Crect fill='%23f5f3ee' width='100%25' height='100%25'/%3E%3Ctext x='50%25' y='50%25' dominant-baseline='middle' text-anchor='middle' fill='%23999' font-size='24'%3ESin imagen%3C/text%3E%3C/svg%3E";
          ...
          <div>
            ...
            <p className="text-2xl font-semibold text-[#4A4A4A] mb-4">
              {producto.precio_producto
                ? `${Number(producto.precio_producto).toFixed(2)} COP`
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {cart.map((item) => (
            <div key={item.id_producto} className="flex gap-4 p-4 bg-gray-50 rounded-xl">
              <img
                src={getImageSrc(item.imagen_producto)}
                alt={item.nombre_producto}
                className="w-20 h-20 object-cover rounded-lg"
              />
              <div className="flex-1">
                <h3 className="font-semibold text-gray-800">{item.nombre_producto}</h3>
                <p className="text-gray-600 text-sm">${Number(item.precio_producto).toFixed(2)} COP</p>
              ...
              <div className="text-right">
                <p className="font-semibold text-gray-800">
                  ${(item.precio_producto * item.cantidad).toFixed(2)} COP
                </p>
              </div>
            </div>
          ))}
