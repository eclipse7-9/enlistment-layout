...
class Usuario(Base): 
    ...
    # domicilios creados por el usuario (cliente)
    domicilios = relationship("Domicilio", back_populates="usuario", foreign_keys="[Domicilio.id_usuario]")
    # domicilios creados por el usuario (cliente)
    # Nota: no se mantiene aquí relación de domicilios asignados porque la nueva
    # estructura de la BD no registra un `id_domiciliario` en la tabla `domicilios`.
...
class Domicilio(Base):
    ...
    id_ciudad = Column(Integer, ForeignKey("ciudades.id_ciudad"), nullable=False)
    id_usuario = Column(Integer, ForeignKey("usuarios.id_usuario"), nullable=False)
    usuario = relationship("Usuario", back_populates="domicilios", foreign_keys=[id_usuario])
    region = relationship("Region")
    ciudad = relationship("Ciudad")
...
class Mensaje(Base):
    ...
    fecha_creacion = Column(TIMESTAMP)
    denunciado = Column(Boolean, default=False)
...
class Bloqueo(Base):
    ...
    id_emprendedor = Column(Integer, ForeignKey("usuarios.id_usuario"), nullable=False)
    id_usuario = Column(Integer, ForeignKey("usuarios.id_usuario"), nullable=False)
...
class Denuncia(Base):
    ...
    id_cita = Column(Integer, ForeignKey("citas.id_cita"), nullable=True)
    id_reportador = Column(Integer, ForeignKey("usuarios.id_usuario"), nullable=False)
    id_objetivo = Column(Integer, ForeignKey("usuarios.id_usuario"), nullable=True)
