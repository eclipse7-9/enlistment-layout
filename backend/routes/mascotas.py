...
def crear_mascota(
    ...
    print("Datos recibidos:", mascota.dict())
    mascota_data = mascota.dict()
    nueva_mascota = Mascota(**mascota_data, id_usuario=current_user.id_usuario)
...
def obtener_mascotas_usuario(
    ...
    return db.query(Mascota).filter(Mascota.id_usuario == current_user.id_usuario).all()
